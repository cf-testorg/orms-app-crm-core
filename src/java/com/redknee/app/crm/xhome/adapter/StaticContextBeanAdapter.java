 ...
 * @since 8.3
 */
public class StaticContextBeanAdapter<BASE extends AbstractBean, CONCRETE extends BASE> extends BeanAdapter
{
    private static final long serialVersionUID = 1L;
    
    
    public StaticContextBeanAdapter(Class<BASE> baseClass, Class<CONCRETE> concreteClass, Context context)
    {
    ...
    {
    	Context appctx = (Context)context_.get("app");
    	if(appctx.getBoolean("UseStaticContextBeanAdapter", false))
    	{
    		if(LogSupport.isDebugEnabled(appctx))
    		{
    			LogSupport.debug(ctx, this, "UseStaticContextBeanAdapter Enabled.");
    		}
    		return super.adapt(context_,bean);
    	}
    	else
    	{
